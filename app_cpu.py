...
orb = cv2.ORB_create(nfeatures=2000) # Initialise ORB detector

#NOTE - On cree un objet bfmatch, avec ca on va trouver les corespondance en comparant les descripteurs
bf = cv2.BFMatcher_create(cv2.NORM_HAMMING) #Brute-Force Matcher
...
def stitchImages(img1, img2):
    ...
    keypoints1, descriptors1 = orb.detectAndCompute(img1, None)# none car il n'y a pas de masque, donc tous les points sont pris
    ...
    MIN_MATCH_COUNT = 10 # je fixe le nbr minim de correspond pour calculer l'homographie
    ...
    if len(good) > MIN_MATCH_COUNT:
        ...
        src_pts_list = []
        for i in good:
            keypoint = keypoints1[i.queryIdx] #keypoint :  < cv2.KeyPoint 0000022435A77480 >
        ...
        dst_pts_list = []
        for i in good:
            keypoint = keypoints2[i.trainIdx]
            point = keypoint.pt
...
end_time = time.time()
print("Temps d'exécution sans CUDA: {:.2f} secondes".format(end_time - start_time))
